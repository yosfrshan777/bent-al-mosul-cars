...
def current_user(authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> User:
    ...
    try:
        payload = decode_token(authorization[7:]); uid = int(payload["sub"])
    except Exception:
...
def car_dict(c: Car):
    return {"id": c.id, "owner_id": c.owner_id, "brand": c.brand, "model": c.model, "year": c.year,
            "price": c.price, "km": c.km, "city": c.city, "fuel": c.fuel, "transmission": c.transmission,
            "description": c.description, "plan": c.plan, "images": json.loads(c.images_json or "[]"),
            "phone": c.phone, "body_type": c.body_type, "status": c.status, "created_at": c.created_at.isoformat()}
...
def cars(db: Session = Depends(get_db)):
    return [car_dict(c) for c in db.scalars(select(Car).where(Car.status == "approved").order_by(Car.created_at.desc())).all()]

@app.post("/api/cars")
def create_car(data: CarCreate, user: User = Depends(current_user), db: Session = Depends(get_db)):
    car = Car(owner_id=user.id, brand=data.brand, model=data.model, year=data.year, price=data.price, km=data.km,
              city=data.city, fuel=data.fuel, transmission=data.transmission, description=data.description,
              plan=data.plan, images_json=json.dumps(data.images, ensure_ascii=False), phone=data.phone or user.phone,
              body_type=data.body_type, status="pending")
    db.add(car); db.commit(); db.refresh(car); return car_dict(car)

@app.get("/api/cars/mine/list")
def my_cars(user: User = Depends(current_user), db: Session = Depends(get_db)):
    return [car_dict(c) for c in db.scalars(select(Car).where(Car.owner_id == user.id).order_by(Car.created_at.desc())).all()]
