...
async function testDatabase() {
  ...
  try {
    ...
    await connection.execute(`CREATE TABLE IF NOT EXISTS cars (id INT NOT NULL AUTO_INCREMENT, user_id INT NOT NULL, brand VARCHAR(100) NOT NULL, model VARCHAR(100) NOT NULL, year INT NOT NULL, price INT NOT NULL, km INT DEFAULT 0, city VARCHAR(100) NOT NULL, fuel VARCHAR(50) DEFAULT 'بنزين', transmission VARCHAR(50) DEFAULT 'أوتوماتيك', description TEXT, plan VARCHAR(30) DEFAULT 'عادي', status VARCHAR(30) DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (id), INDEX idx_cars_user_id (user_id), INDEX idx_cars_status (status), CONSTRAINT fk_cars_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`);
