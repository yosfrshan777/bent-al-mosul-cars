...
router.get('/', async (_, res) => {
  try {
    const [rows] = await pool.execute(`SELECT id,name,phone,city,created_at FROM users WHERE role = 'showroom' ORDER BY id DESC`);
    res.json(rows);
  } catch (error) { console.error('SHOWROOMS ERROR:', error); res.status(500).json({message:'تعذر تحميل المعارض'}); }
});

router.get('/:id', async (req, res) => {
  try {
    const id = Number(req.params.id); if (!Number.isInteger(id)) return res.status(400).json({message:'رقم المعرض غير صحيح'});
    const [users] = await pool.execute(`SELECT id,name,phone,city,created_at FROM users WHERE id = ? AND role = 'showroom' LIMIT 1`, [id]);
  ...
  } catch (error) { console.error('SHOWROOM DETAILS ERROR:',error); res.status(500).json({message:'تعذر تحميل بيانات المعرض'}); }
});

router.post('/request', auth, async (req, res) => {
  try {
    const name=String(req.body.name||'').trim(), phone=String(req.body.phone||'').trim(), city=String(req.body.city||'').trim();
  ...
  } catch(error){console.error('SHOWROOM REQUEST ERROR:',error);res.status(500).json({message:'تعذر إرسال طلب المعرض'});}
});

router.put('/:id/approve', auth, admin, async (req,res)=>{
  try{
    const id=Number(req.params.id); if(!Number.isInteger(id))return res.status(400).json({message:'رقم المستخدم غير صحيح'});
  ...
  }catch(error){console.error('APPROVE SHOWROOM ERROR:',error);res.status(500).json({message:'تعذر اعتماد المعرض'});}
});
