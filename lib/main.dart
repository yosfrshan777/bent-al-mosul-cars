        ...
        body: Stack(fit: StackFit.expand, children: [
          ...
          Center(child: AnimatedBuilder(animation: _animation, builder: (_, __) {
            ...
            return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Transform.scale(scale: .88 + (v.clamp(0.0, 1.0) * .12), child: ShaderMask(shaderCallback: (b) => const LinearGradient(colors: [kPink, Colors.white, kBlue]).createShader(b), child: const Text('ZYOCAR', style: TextStyle(color: Colors.white, fontSize: 42, fontWeight: FontWeight.w900, letterSpacing: 5)))),
              const SizedBox(height: 10),
              const Text('بيع وشراء السيارات وقطع الغيار', style: TextStyle(color: Colors.white70, fontSize: 13)),
              const SizedBox(height: 38),
              SizedBox(height: 220, width: 340, child: AnimatedOpacity(opacity: v.clamp(0.0, 1.0), duration: const Duration(milliseconds: 250), child: Image.asset('assets/icons/zyocar.png', fit: BoxFit.contain, errorBuilder: (_, __, ___) => const SizedBox.shrink()))),
...
class _AuthGateState extends State<AuthGate> {
  ...
  Future<void> _checkSession() async {
    ...
    if (!mounted) return;
    setState(() { _loggedIn = valid; _checking = false; });
  }
...
class LoginRequiredScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: kBg, body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
    const Icon(Icons.lock_person_rounded, color: kPink, size: 72), const SizedBox(height: 18),
    const Text('تسجيل الدخول مطلوب', style: TextStyle(color: Colors.white, fontSize: 25, fontWeight: FontWeight.w900)), const SizedBox(height: 8),
    const Text('سجّل دخولك حتى تدخل إلى ZYOCAR', textAlign: TextAlign.center, style: TextStyle(color: Colors.white54)), const SizedBox(height: 24),
...
class _MainShellState extends State<MainShell> {
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      HomeScreen(api: widget.api, onOpenCars: () => setState(() => _index = 1), onOpenShowrooms: _showrooms, onOpenParts: _parts, onAddCar: _addCar),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(selectedIndex: _index, onDestinationSelected: (value) => setState(() => _index = value), backgroundColor: const Color(0xFF0D1018), indicatorColor: const Color(0xFF3A1630), height: 78, destinations: const [
