...
class _AuthGateState extends State<AuthGate> {
  ...
  Future<void> check() async { var valid = widget.api.isLoggedIn; if (valid) { try { await widget.api.me(); } catch (_) { valid = false; } } if (!mounted) return; setState(() { loggedIn = valid; checking = false; }); }
  Future<void> login() async { final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => LoginScreen(api: widget.api))); if (result != null && mounted) setState(() => loggedIn = true); }
  @override Widget build(BuildContext context) { if (checking) return const Scaffold(body: Center(child: CircularProgressIndicator(color: kPink))); if (loggedIn) return MainShell(api: widget.api); return LoginRequiredScreen(onLogin: login); }
}
