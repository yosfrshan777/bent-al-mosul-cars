...
class _AuthGateState extends State<AuthGate> {
  ...
  Widget build(BuildContext context) {
    if (checking) {
      return const Scaffold(
        body: Center(child: CircularProgressIndicator(color: kPink)),
      );
    }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'سجّل دخولك حتى تدخل إلى ZYOCAR',
...
class _MainShellState extends State<MainShell> {
  ...
  void showrooms() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => ShowroomsScreen(api: widget.api)),
    );
  }
  ...
  void parts() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => PartsScreen(api: widget.api)),
    );
  }
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      ZyoCarHomeScreen(
        api: widget.api,
        onOpenCars: () => setState(() => index = 1),
        onOpenShowrooms: showrooms,
        onOpenParts: parts,
        onAddCar: addCar,
      ),
