...
class ApiService {
  ...
  final String baseUrl; String? _token;
  ...
  Future<dynamic> login({required String username,required String phone,required String password})async{final d=await _request('POST','/auth/login',body:{'username':username.trim(),'phone':phone.trim(),'password':password});if(d is Map&&(d['token']??d['access_token'])!=null)await setToken((d['token']??d['access_token']).toString());return d;}
  ...
  Future<dynamic> me()=>_request('GET','/auth/me'); Future<void> logout()async{try{await _request('POST','/auth/logout');}finally{await clearToken();}}
  ...
  Future<dynamic> createCar({required String brand,required String model,required int year,required int price,required int km,required String city,required String fuel,required String transmission,required String description,required String plan,required List<XFile> images,String phone='',String? bodyType})async{
    ...
    try{
      final q=http.MultipartRequest('POST',Uri.parse('$baseUrl/upload/car-images'));q.headers.addAll(_headers);
      ...
      final urls=<String>[];if(d is Map&&d['images'] is List){for(final x in d['images'])if(x is Map&&x['url']!=null)urls.add(x['url'].toString());}
  ...
  Future<dynamic> getShowrooms()=>_request('GET','/showrooms'); Future<dynamic> getShowroom(int id)=>_request('GET','/showrooms/$id'); Future<dynamic> requestShowroom({required String name,required String phone,required String city})=>_request('POST','/showrooms/request',body:{'name':name,'phone':phone,'city':city});
  Future<dynamic> getParts()=>_request('GET','/parts'); Future<dynamic> getPartStore(int id)=>_request('GET','/parts/$id'); Future<dynamic> requestParts({required String name,required String phone,required String city})=>_request('POST','/parts/request',body:{'name':name,'phone':phone,'city':city});
  Future<dynamic> getAdminData()=>_request('GET','/admin/dashboard'); Future<dynamic> getPendingRequests()=>_request('GET','/admin/requests/pending'); Future<dynamic> approveRequest(int id)=>_request('POST','/admin/requests/$id/approve'); Future<dynamic> rejectRequest(int id)=>_request('POST','/admin/requests/$id/reject');
  Future<dynamic> getPaymentSettings()=>_request('GET','/payment/settings'); Future<dynamic> updatePaymentSettings({required String phone,required String cardNumber,required String accountName,String? method})=>_request('PUT','/admin/payment-settings',body:{'phone':phone,'card_number':cardNumber,'account_name':accountName,'method':method??'card'});
