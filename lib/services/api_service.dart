...
class ApiService {
  ...
  Map<String, String> get _headers {
    ...
    if (isLoggedIn) headers['Authorization'] = 'Bearer $_token';
    return headers;
  }
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        if (data is Map) {
          ...
          if (value != null) message = value.toString();
        }
      ...
      throw const ApiException('لا يوجد اتصال بالسيرفر');
    } on HttpException {
      throw const ApiException('تعذر الاتصال بالسيرفر');
    } on ApiException {
      rethrow;
    } catch (_) {
      throw const ApiException('حدث خطأ أثناء الاتصال بالسيرفر');
    }
  }
  ...
  Future<dynamic> login({required String phone, required String password}) async {
    ...
    if (data is Map) {
      ...
      if (token != null) await setToken(token.toString());
    }
    return data;
  }

  Future<dynamic> register({required String name, required String phone, required String password}) async {
  ...
  Future<List<dynamic>> getCars() async {
    final data = await _request('GET', '/cars');
    if (data is List) return data;
    if (data is Map && data['cars'] is List) return data['cars'];
    return [];
  }
  ...
  Future<dynamic> createCar({
    ...
    try {
      final uploadRequest = http.MultipartRequest('POST', Uri.parse('$baseUrl/upload/car-images'));
      uploadRequest.headers.addAll(_headers);
      for (final image in images) {
        uploadRequest.files.add(await http.MultipartFile.fromPath('images', image.path, filename: image.name));
      }
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        if (uploadData is Map) {
          final value = uploadData['message'] ?? uploadData['error'];
          if (value != null) message = value.toString();
        }
        throw ApiException(message, statusCode: response.statusCode);
      }
      ...
      if (uploadData is Map && uploadData['images'] is List) {
        for (final item in uploadData['images']) {
          if (item is Map && item['url'] != null) imageUrls.add(item['url'].toString());
        }
      }
      if (imageUrls.isEmpty) throw const ApiException('تم رفع الصور لكن لم يتم استلام روابط الصور');

      return _request('POST', '/cars', body: {
        'brand': brand,
        'model': model,
        'year': year,
        'price': price,
      ...
      throw const ApiException('حدث خطأ أثناء نشر السيارة');
    }
  }
  ...
  Future<dynamic> updatePaymentSettings({required String phone, required String cardNumber, required String accountName, String? method}) =>
      _request('PUT', '/admin/payment-settings', body: {'phone': phone, 'card_number': cardNumber, 'account_name': accountName, 'method': method ?? 'card'});

  Future<dynamic> updatePlanPrices({required int normal, required int featured, required int vip}) =>
      _request('PUT', '/admin/plan-prices', body: {'normal': normal, 'featured': featured, 'vip': vip});

  Future<dynamic> getShowrooms() => _request('GET', '/showrooms');
  Future<dynamic> getShowroom(int id) => _request('GET', '/showrooms/$id');
  Future<dynamic> requestShowroom({required String name, required String phone, required String city}) =>
      _request('POST', '/showrooms/request', body: {'name': name, 'phone': phone, 'city': city});

  Future<dynamic> getParts() => _request('GET', '/parts');
  Future<dynamic> getPartStore(int id) => _request('GET', '/parts/$id');
  Future<dynamic> requestParts({required String name, required String phone, required String city}) =>
      _request('POST', '/parts/request', body: {'name': name, 'phone': phone, 'city': city});
  ...
  Future<dynamic> sendMessage({required int receiverId, required String text}) =>
      _request('POST', '/messages', body: {'receiver_id': receiverId, 'text': text});
  ...
  Future<dynamic> createPayment({required int amount, required String method, String? phone, String? cardNumber, String? accountName, String? reference}) =>
      _request('POST', '/payment', body: {'amount': amount, 'method': method, 'phone': phone, 'card_number': cardNumber, 'account_name': accountName, 'reference': reference});
  ...
  String imageUrl(String path) {
    ...
    if (path.startsWith('/')) return baseUrl.replaceFirst('/api', '') + path;
