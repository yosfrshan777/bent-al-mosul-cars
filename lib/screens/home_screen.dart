...
const Color _bg = Color(0xFF07090F);
const Color _card = Color(0xFF10141D);
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _price(int value) {
    ...
    final formatted = text.replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+(?!\d))'), (m) => '${m[1]},');
    return '\$$formatted';
  }
  ...
  Widget _header() {
    return Row(
      children: [
        Container(width: 42, height: 42, decoration: BoxDecoration(color: _card, borderRadius: BorderRadius.circular(13), border: Border.all(color: _pink.withOpacity(.55))), child: const Icon(Icons.notifications_none_rounded, color: Colors.white)),
        const Spacer(),
        ...
        const Spacer(),
        Container(width: 42, height: 42, decoration: BoxDecoration(color: _card, borderRadius: BorderRadius.circular(13)), child: const Icon(Icons.search_rounded, color: Colors.white)),
      ],
    );
  }
  ...
  Widget _hero() {
    return Container(
      ...
      child: Stack(children: [
        ...
        const Positioned(left: 145, bottom: 18, child: Icon(Icons.directions_car_filled_rounded, size: 105, color: _pink)),
        Positioned(left: 88, bottom: 28, child: Icon(Icons.auto_awesome_rounded, size: 28, color: _blue.withOpacity(.75))),
      ]),
    );
  }
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), border: Border.all(color: color.withOpacity(.35))),
  ...
  Widget _sectionTitle(String title) {
    return Row(children: [
      ...
      TextButton(onPressed: widget.onOpenCars, child: const Text('عرض الكل', style: TextStyle(color: _pink))),
    ]);
  }
  ...
  Widget _carCard(Car car) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: _card, borderRadius: BorderRadius.circular(20), border: Border.all(color: car.isVip ? _pink.withOpacity(.75) : const Color(0xFF252B38))),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          SizedBox(
            ...
            child: Stack(fit: StackFit.expand, children: [
              car.image == null || car.image!.isEmpty
                  ? const Center(child: Icon(Icons.directions_car_filled_rounded, color: _pink, size: 70))
                  : Image.network(widget.api.imageUrl(car.image!), fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.directions_car_filled_rounded, color: _pink, size: 70))),
              if (car.isVip)
                Positioned(top: 9, right: 9, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: _pink, borderRadius: BorderRadius.circular(15)), child: const Text('VIP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 10)))),
            ]),
          ),
          Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(_price(car.price), style: const TextStyle(color: _pink, fontSize: 17, fontWeight: FontWeight.w900)),
          ...
          child: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                _sectionTitle('تصنف حسب الماركة'),
                SizedBox(height: 58, child: ListView(scrollDirection: Axis.horizontal, children: const [
                  _BrandChip('تويوتا'), _BrandChip('هيونداي'), _BrandChip('بي إم دبليو'), _BrandChip('مرسيدس'), _BrandChip('كيا'),
                ])),
                const SizedBox(height: 18),
                _sectionTitle('سيارات مميزة'),
                _latest(),
              ],
            ),
          ),
...
class _BrandChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: _card, borderRadius: BorderRadius.circular(17), border: Border.all(color: const Color(0xFF273040))),
      child: Row(children: [const Icon(Icons.directions_car_filled_rounded, color: Colors.white70, size: 23), const SizedBox(width: 7), Text(name, style: const TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.bold))]),
    );
  }
}
