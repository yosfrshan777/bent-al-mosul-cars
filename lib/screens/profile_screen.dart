...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  @override Widget build(BuildContext context) {
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: const Color(0xFF08080B), appBar: AppBar(title: const Text('حسابي', style: TextStyle(fontWeight: FontWeight.w900)), centerTitle: true), body: loading ? const Center(child: CircularProgressIndicator(color: Color(0xFFFF176F))) : ListView(padding: const EdgeInsets.all(16), children: [
      Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topRight, end: Alignment.bottomLeft, colors: [Color(0xFF321222), Color(0xFF15151B)]), borderRadius: BorderRadius.circular(21), border: Border.all(color: const Color(0xFF3A2631))), child: Row(children: [Container(width: 65, height: 65, decoration: const BoxDecoration(color: Color(0xFFFF176F), shape: BoxShape.circle), child: const Icon(Icons.person_rounded, color: Colors.white, size: 34)), const SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name(), style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)), if (phone().isNotEmpty) Padding(padding: const EdgeInsets.only(top: 4), child: Text(phone(), style: const TextStyle(color: Colors.white54))), const SizedBox(height: 5), Text(role(), style: const TextStyle(color: Color(0xFFFF4F91), fontWeight: FontWeight.bold))]))])),
      ...
      const SizedBox(height: 25), const Center(child: Text('ZYOCAR', style: TextStyle(color: Colors.white24, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: 2))),
    ]));
  }
}
