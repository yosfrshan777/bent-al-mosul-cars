...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    try {
      final d = await widget.api.me();
      if (!mounted) return;
      if (d is Map && d['user'] is Map) {
        setState(() => user = Map<String, dynamic>.from(d['user']));
      }
    } catch (_) {
    } finally {
  ...
  Future<void> login() async {
    final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => LoginScreen(api: widget.api)));
    if (r != null) await _load();
  }

  Future<void> logout() async {
    try { await widget.api.logout(); } catch (_) { await widget.api.clearToken(); }
    if (!mounted) return;
    setState(() => user = null);
    _msg('تم تسجيل الخروج');
  }
  ...
  Widget item({required IconData icon, required String title, required VoidCallback onTap, String? subtitle}) {
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      decoration: BoxDecoration(color: const Color(0xFF15151B), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFF292932))),
      child: ListTile(
        onTap: onTap,
        leading: Container(width: 43, height: 43, decoration: BoxDecoration(color: const Color(0xFF28141F), borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: const Color(0xFFFF176F))),
