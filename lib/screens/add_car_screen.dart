...
class _AddCarScreenState extends State<AddCarScreen> {
  ...
  final Map<String,List<String>> models = const {
    'تويوتا':['كامري','كورولا','لاندكروزر','راف فور','هايلاندر','برادو','يارس','تندرا','تاكوما','أفالون','إنوفا'], 'هيونداي':['توسان','سوناتا','إلنترا','سانتافي','كونا','أكسنت','باليسايد','كريتا'], 'بي إم دبليو':['X5','X3','X6','الفئة الثالثة','الفئة الخامسة','الفئة السابعة','X1','X7'], 'مرسيدس':['C-Class','E-Class','S-Class','GLE','GLC','G-Class','A-Class'], 'كيا':['سبورتاج','سورينتو','K5','سيراتو','سيلتوس','كارنفال','ريو'], 'نيسان':['باترول','ألتيما','سنترا','إكس تريل','صني','كيكس','نافارا'], 'أودي':['A4','A6','Q5','Q7','Q8','A3'], 'فورد':['إكسبلورر','إيدج','إف-150','موستانغ','إسكيب','إكسبيديشن'], 'شيفروليه':['تاهو','سوبربان','ماليبو','كابتيفا','سيلفرادو','كولورادو'], 'مازدا':['CX-5','CX-9','6','3','CX-30'], 'هوندا':['أكورد','سيفيك','CR-V','بايلوت','HR-V'], 'لكزس':['LX','GX','RX','ES','IS','NX'], 'ميتسوبيشي':['باجيرو','أوتلاندر','ASX','L200'], 'جيب':['جراند شيروكي','رانجلر','كومباس','جلاديتور'], 'فولكس فاجن':['تيرامونت','تيغوان','جيتا','باسات'], 'بورشه':['كاين','ماكان','911','باناميرا'], 'تسلا':['Model 3','Model Y','Model S','Model X'], 'سوزوكي':['سويفت','فيتارا','جيمني','سويفت ديزاير'], 'رينو':['داستر','كوليوس','ميغان'], 'بيجو':['3008','5008','508','2008'], 'شيري':['تيجو 7','تيجو 8','أريزو 5'], 'MG':['ZS','RX5','5','6'], 'جيلي':['كولراي','أمجراند','أوكافانغو'], 'هافال':['جوليان','H6','H9'], 'BYD':['سونج','هان','سيل','دولفين'],
  };
  String? brand, model, city, bodyType; int? year; String fuel='بنزين', transmission='أوتوماتيك', plan='عادي'; List<XFile> images=[]; bool loading=false;
  ...
  Future<void> chooseImages() async { final picked = await picker.pickMultiImage(imageQuality: 88, maxWidth: 1800, maxHeight: 1800); if (picked.isEmpty) return; setState(() => images = picked.take(5).toList()); if (picked.length > 5) _msg('تم اعتماد أول 5 صور فقط'); }
  ...
  Future<void> submit() async {
    ...
    try {
      final result = await widget.api.createCar(brand: brand!, model: model!, year: year!, price: p, km: 0, city: city!, fuel: fuel, transmission: transmission, description: description.text.trim(), plan: plan, images: images, phone: phone.text.trim(), bodyType: bodyType);
      if (!mounted) return;
