...
class _AddCarScreenState extends State<AddCarScreen> {
  ...
  final Map<String,List<String>> models = const {
    ...
    'مازدا':['CX-5','CX-9','6','3','CX-30'], 'هوندا':['أكورد','سيفيك','CR-V','بايلوت','HR-V'],
    'لكزس':['LX','GX','RX','ES','IS','NX'], 'ميتسوبيشي':['باجيرو','أوتلاندر','ASX','L200'],
    'جيب':['جراند شيروكي','رانجلر','كومباس','جلاديتور'], 'فولكس فاجن':['تيرامونت','تيغوان','جيتا','باسات'],
    'بورشه':['كاين','ماكان','911','باناميرا'], 'تسلا':['Model 3','Model Y','Model S','Model X'],
    'سوزوكي':['سويفت','فيتارا','جيمني','سويفت ديزاير'], 'رينو':['داستر','كوليوس','ميغان'],
    'بيجو':['3008','5008','508','2008'], 'شيري':['تيجو 7','تيجو 8','أريزو 5'], 'MG':['ZS','RX5','5','6'],
    'جيلي':['كولراي','أمجراند','أوكافانغو'], 'هافال':['جوليان','H6','H9'], 'BYD':['سونج','هان','سيل','دولفين'],
  };
  ...
  Future<void> submit() async {
    ...
    try {
      final result = await widget.api.createCar(brand: brand!, model: model!, year: year!, price: p, km: 0, city: city!, fuel: fuel, transmission: transmission, description: description.text.trim(), plan: plan, images: images, phone: phone.text.trim(), bodyType: bodyType, category: null);
  ...
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: const Color(0xFF05070D), appBar: AppBar(title: const Text('بيع سيارة', style: TextStyle(fontWeight: FontWeight.w900)), centerTitle: true, backgroundColor: const Color(0xFF080C14)), body: Form(key: formKey, child: ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 32), children: [
    ...
    const SizedBox(height: 5), const Text('اختر من صورتين إلى 5 صور', style: TextStyle(color: Colors.white54, fontSize: 11)), const SizedBox(height: 10),
