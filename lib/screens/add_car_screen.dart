...
class _AddCarScreenState extends State<AddCarScreen> {
  ...
  final Map<String,List<String>> models = const {
    'تويوتا':['كامري','كورولا','لاندكروزر','راف فور','هايلاندر','برادو','يارس'],
    'هيونداي':['توسان','سوناتا','إلنترا','سانتافي','كونا','أكسنت'],
    'بي إم دبليو':['X5','X3','X6','الفئة الثالثة','الفئة الخامسة','الفئة السابعة'],
    'مرسيدس':['C-Class','E-Class','S-Class','GLE','GLC','G-Class'],
    'كيا':['سبورتاج','سورينتو','K5','سيراتو','سيلتوس','كارنفال'],
    'نيسان':['باترول','ألتيما','سنترا','إكس تريل','صني'],
    'أودي':['A4','A6','Q5','Q7','Q8'],
    'فورد':['إكسبلورر','إيدج','إف-150','موستانغ','إسكيب'],
    'شيفروليه':['تاهو','سوبربان','ماليبو','كابتيفا','سيلفرادو'],
    'مازدا':['CX-5','CX-9','6','3'],
  ...
  InputDecoration dec(String label, {String? hint}) => InputDecoration(
    ...
    filled: true, fillColor: const Color(0xFF111722),
    prefixIcon: const Icon(Icons.keyboard_arrow_down_rounded, color: Colors.white38),
    border: OutlineInputBorder(borderRadius: BorderRadius.circular(17), borderSide: const BorderSide(color: Colors.white12)),
    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(17), borderSide: const BorderSide(color: Colors.white12)),
    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(17), borderSide: const BorderSide(color: Color(0xFFFF176F), width: 1.4)),
  );

  Widget select<T>({required String label, required T? value, required List<T> items, required String Function(T) text, required ValueChanged<T?> onChanged}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: DropdownButtonFormField<T>(
        value: value, dropdownColor: const Color(0xFF101722), isExpanded: true,
        decoration: dec(label), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700),
        items: items.map((x) => DropdownMenuItem<T>(value: x, child: Text(text(x), overflow: TextOverflow.ellipsis))).toList(),
        onChanged: onChanged,
  ...
  Widget input(TextEditingController c, String label, {TextInputType? type, bool required = false, int maxLines = 1}) => Padding(
    padding: const EdgeInsets.only(bottom: 12),
    child: TextFormField(
      controller: c, keyboardType: type, maxLines: maxLines, textDirection: TextDirection.rtl,
      style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700),
      decoration: dec(label).copyWith(prefixIcon: const Icon(Icons.edit_outlined, color: Colors.white38)),
  ...
  Future<void> analyze() async {
    ...
    try {
      ...
      if (!mounted) return;
      final aiBrand = a['brand']?.toString();
      final aiModel = a['model']?.toString();
      final aiYear = int.tryParse('${a['year'] ?? ''}');
      final aiFuel = a['fuel']?.toString();
      final aiTransmission = a['transmission']?.toString();
      setState(() {
  ...
  Widget build(BuildContext context) => Directionality(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(title: const Text('إضافة إعلان سيارة', style: TextStyle(fontWeight: FontWeight.w900)), centerTitle: true, backgroundColor: const Color(0xFF080C14)),
      body: Form(key: formKey, child: ListView(padding: const EdgeInsets.fromLTRB(16, 16, 16, 32), children: [
        Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF27102B), Color(0xFF0D1A2A)]), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFFFF176F).withOpacity(.35))), child: Row(children: [
          Container(width: 54, height: 54, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFFFF176F).withOpacity(.15)), child: const Icon(Icons.auto_awesome_rounded, color: Color(0xFFFF176F), size: 30)),
          const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [Text('ZYOCAR AI', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('صوّر السيارة وخلي الذكاء الاصطناعي يساعدك بالبيانات', style: TextStyle(color: Colors.white60, fontSize: 10))])),
          IconButton(onPressed: analyzing ? null : analyze, icon: analyzing ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFFFF176F))) : const Icon(Icons.camera_enhance_rounded, color: Colors.white)),
        ])),
        const SizedBox(height: 18),
        const Text('صور السيارة', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
        const SizedBox(height: 5),
        const Text('اختر من صورتين إلى 5 صور — هذا الحقل إجباري', style: TextStyle(color: Colors.white54, fontSize: 11)),
        ...
        input(km, 'المسافة بالكيلومتر', type: TextInputType.number),
