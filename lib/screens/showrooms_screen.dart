...
class _ShowroomsScreenState extends State<ShowroomsScreen> {
  ...
  @override Widget build(BuildContext context) {
    Widget body;
    if (loading) body = ListView(children: const [SizedBox(height: 220), Center(child: CircularProgressIndicator(color: _pink))]);
    else if (error != null) body = ListView(children: [const SizedBox(height: 160), const Center(child: Text('تعذر الاتصال بالسيرفر', style: TextStyle(color: Colors.white70))), Center(child: TextButton(onPressed: _load, child: Text(error!)))]);
    else if (items.isEmpty) body = ListView(children: const [SizedBox(height: 160), Center(child: Text('لا توجد معارض معتمدة حالياً', style: TextStyle(color: Colors.white54)))]);
  ...
  Widget _card(Map<String, dynamic> x) => Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: _blue.withOpacity(.25))), child: ListTile(
...
class _ShowroomDetailsScreenState extends State<ShowroomDetailsScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        ...
        cars = rc is List ? rc.whereType<Map>().map((e) => Map<String, dynamic>.from(e)).toList() : [];
        loading = false;
      });
  ...
  @override Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: _bg, appBar: AppBar(title: const Text('صفحة المعرض', style: TextStyle(fontWeight: FontWeight.w900)), backgroundColor: _bg), body:
      loading ? const Center(child: CircularProgressIndicator(color: _pink)) : error != null ? Center(child: Text(error!, style: const TextStyle(color: Colors.white54))) : ListView(padding: const EdgeInsets.all(15), children: [
